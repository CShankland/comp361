...
void chomp(char array[]) // replace \r and \n with \0
{
	int i;
	for(i=0;array[i]!='\0';i++)
...
int glmFindOrAddTexture(GLMmodel* model, char* name,mycallback *call)
{
	...
	//	  model->numtextures, model->textures[model->numtextures-1].name,  model->textures[model->numtextures-1].id,
	//	   model->textures[model->numtextures-1].width,model->textures[model->numtextures-1].height);
